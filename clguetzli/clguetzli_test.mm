...
int floatCompare(const float* a, const float* b, size_t size, const char* szFunc, int line)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        {
            count++;
            //NSLog(@"error index :%d  value A:%f  ,b:%f\r\n",i,a[i],b[i] );
            
        }
        if(count > 0 && i % 5000 == 2 )
        {
            NSLog(@"error index :%d  value CPU :%f  ,GPU :%f\r\n",i,a[i],b[i] );
        }
    }
    if (count > 0)
    {
        NSLog(@"CHKERROR %s(%d) %d:%d\r\n", szFunc, line, count, size);
    }

    NSLog(@"CHKALL %s(%d) %d:%d\r\n", szFunc, line, count, size);
    NSLog(@"show index :%d  value CPU :%f  ,GPU :%f\r\n",100,a[100],b[100] );
                     ...
                     const float* result_diff_dc, const float* result_diff_ac)
{
    ...
    FLOAT_COMPARE(result_diff_dc, r_dc, res_xsize * res_ysize * 3);
    FLOAT_COMPARE(result_diff_ac, r_ac, res_xsize * res_ysize * 3);
                        ...
                        const float *result)
{
    ...
    FLOAT_COMPARE( result,result_tmp, res_xsize * res_ysize);
    delete [] result_tmp;
                         ...
                         const float *diffmap_cmp)
{
    ...
    FLOAT_COMPARE( diffmap_cmp, result_tmp,xsize * ysize);
...
//    NSLog(@"sum: %f",xyb1[0][100]);
//    NSLog(@"sum: %f",xyb1[1][100]);
//    NSLog(@"sum: %f",xyb1[2][100]);
//    NSLog(@"sum: %f",xyb1[1][1000]);
    ...
    FLOAT_COMPARE((*mask_cmp)[0].data(),r_x,  xsize * ysize);
    FLOAT_COMPARE((*mask_cmp)[1].data(),r_y,  xsize * ysize);
    FLOAT_COMPARE((*mask_cmp)[2].data(),r_b,  xsize * ysize);
    delete [] r_x;
    delete [] r_y;
    delete [] r_b;
...
void tclAverage5x5(int xsize, int ysize, const std::vector<float> &diffs_org, const std::vector<float> &diffs_cmp)
{
    ...
    FLOAT_COMPARE(diffs_cmp.data(),r,  xsize * ysize);
    delete []r;
                     ...
                     const float *result)
{
    ...
    r = clMinSquareValEx(r, xsize, ysize, square_size, offset,0);
    
    cl_float *r_r = new float[img_size];
...
void tclScaleImage(double scale, const float *result_org, const float *result_cmp, size_t length)
{
    ...
    FLOAT_COMPARE( result_cmp, r_r,length);
    delete [] r_r;
